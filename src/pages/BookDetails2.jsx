...
const BookDetails2 = () => {
  ...
  const Filter = async () => {
    try {
      const response = await fetch("https://library-management-api-backup.onrender.com/books");
  ...
  if (loading) return <p className="text-center text-gray-400">Loading book details...</p>;
  ...
  return (
    // <PagesLayouts>
      <div className=" bookdetails min-h-screen py-12 px-6 bg-gray-100">
        <div className="max-w-5xl mx-auto bg-white  shadow-md overflow-hidden md:flex">
          {/* Image Section */}
          ...
          <div className="md:w-1/2 p-6">
            ...
            {/* <p className="text-gray-600 mb-2"><strong>ID:</strong> {book.id}</p> */}
            <p className="text-gray-700 mb-4">{book.summary}</p>
            <p className="text-pink-600 text-lg font-semibold mb-4">{book.author}</p>
            {book.category && (
              <p className="text-gray-500 mb-2"><strong>Category:</strong> {book.category}</p>
            )}
            {book.category && (
              <p className="text-gray-500 mb-2"><strong>Category:</strong> {book.category}</p>
            )}
            {book.brand && (
              <p className="text-gray-500 mb-2"><strong>Brand:</strong> {book.genre}</p>
            )}
            {book.category && (
              <p className="text-gray-500 mb-2"><strong>Category:</strong> {book.category}</p>
            )}
